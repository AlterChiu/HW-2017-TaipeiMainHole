...
public class RainfallCheckhour {
				...
				for(int month = 1;month<=12;month++){
					...
					for(int day = 1;day<=31;day++){
						...
						for(int hour = 0 ; hour<=23 ; hour++){
							...
							String time = yearS + monthS + dayS + hourS;
							ArrayList<Double> hourTemp = new ArrayList<Double>();
							for(String[] line:content){
								if(line[0].substring(0,10).equals(time)){
									hourTemp.add(Double.parseDouble(line[1]));
								}
							}
							if(new AtCommonMath(hourTemp).getSum()>hourList[month]){
